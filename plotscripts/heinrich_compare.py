...
parser.add_argument('netcdf', type=str, nargs='+')
...
for ncName in args.netcdf:
    ...
    basal_yield = data.variables['thk'][:] / 1000  # kPa
    ...
    for i in range(nRegions):
        ...
        ax1.plot(time, thickness, label=expName,  lw=0.6)
        ax2.plot(time, tillwat, lw=0.6)
        ax3.plot(time, yield_stress, lw=0.6)
        ax4.plot(time, velocity, lw=0.6)
