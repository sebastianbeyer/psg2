...
# uvelsurf = data.variables['uvelsurf'][:]
# vvelsurf = data.variables['vvelsurf'][:]
...
data = xr.open_dataset(args.icemodel)
x = data["x"]
y = data["y"]

thk = data["thk"]
# vel = data["velsurf_mag"]
uvelsurf = data['uvelsurf']
vvelsurf = data['vvelsurf']
mask = data['mask']
# time = data['time']
...
vel = vel[:, firsty:lasty, firstx:lastx]
thk = thk[:, firsty:lasty, firstx:lastx]

uvelsurf = uvelsurf[:, firsty:lasty, firstx:lastx]
vvelsurf = vvelsurf[:, firsty:lasty, firstx:lastx]
mask = mask[:, firsty:lasty, firstx:lastx]
# temppabase = temppabase[:, firsty:lasty, firstx:lastx]
X, Y = np.meshgrid(x, y)
thk_last = thk[args.timestep, :, :]
# usurf_last = usurf[-1, :, :]
usurf_last = thk_last
vel_last = vel[args.timestep, :, :]
mask_last = mask[args.timestep, :, :]

uvel_last = uvelsurf[args.timestep, :, :]
vvel_last = vvelsurf[args.timestep, :, :]

# make regular numpy arrays
uvel_last = uvel_last.to_numpy()
vvel_last = vvel_last.to_numpy()
thk_last = thk_last.to_numpy()
# vel_last = vel_last.to_numpy()
mask_last = mask_last.to_numpy()

# mask = data['mask']

# time = time.to_numpy()

uvel_last[mask_last != 2] = np.NaN
vvel_last[mask_last != 2] = np.NaN

# lic_result = lic.lic(uvel_last, vvel_last, length=30)
...
cmap_GRN = cm.get_cmap('turbo', 210)  # 11 discrete colors

fig, axes = plt.subplots(1, 1, subplot_kw={'projection': crs}, figsize=(6, 6))


# axes.set_extent(netcdfExtend, crs=crs)
# extent = [-3000000, 0000000, -5000000, 0000000]
axes.set_extent(netcdfExtend, crs=crs)
...
if args.variable == "vel":
    ...
    cbThkObs = plt.colorbar(imgThkObs, ticks=ticks,
                            ax=axes, shrink=0.85, pad=0.08)
    cbThkObs.ax.set_yticklabels(tickval)
    # cb = plt.colorbar(fig, ticks=255*(t - t[0])/(t[-1] - t[0]), shrink=0.5)
    cbThkObs.set_label('Ice Surface Velocity (m/a)')

    axes.streamplot(x, y, uvel_last, vvel_last,
                    transform=crs, density=6, arrowstyle='-', color='k', linewidth=0.6)
    # lic is not really sharp :(
    # licimage = axes.imshow(lic_result,
    #                        transform=crs,
    #                        extent=netcdfExtend,
    #                        cmap='Greys',
    #                        origin='lower',
